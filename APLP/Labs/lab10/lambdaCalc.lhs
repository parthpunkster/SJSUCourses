...
> test3mul2 = transChurchNums $ multiply three two
...
To understand how this works, write out the evaluation steps for `factorial 3`.
   factorial three
-> (fix g) three
-> ((\f -> (\x -> f (\y -> x x y)) (\x -> f (\y -> x x y))) g) three
-> ((\x -> g (\y -> x x y)) (\x -> g (\y -> x x y))) three
-> (g (\y -> (\x -> g (\y -> x x y)) (\x -> g (\y -> x x y)) y)) three
-> (\n -> (test (isZero (prd n)) one (multiply n ((\x -> g (\y -> x x y)) (\x -> g (\y -> x x y)) y) (prd n))))) three
-> (test (isZero (prd three)) one (multiply three ((\x -> g (\y -> x x y)) (\x -> g (\y -> x x y)) y) (prd three)))
